---
layout: post
title: "Сходимость в имитации отжига"
...
**Имитация отжига** (отжиг) - основной алгоритм в контестах на оптимизацию. Почти каждый победитель Topcoder Marathon или Atcoder Heuristic Contest скорее всего когда-то писал отжиг. В интернете есть много статей про этот алгоритм, поэтому я предполагаю, что вы уже знакомы с основами. Я рекомендую [вот эту](https://codeforces.com/blog/entry/94437). _(прим.: она на английском, подойдёт и [статья](https://algorithmica.org/ru/annealing) с Алгоритмики)._
...
_Дисклеймер: У меня самого не так много опыта в оптимизационных контестах, поэтому ко всему сказанному здесь стоит относиться с осторожностью. Но тем не менее, я был членом команды, которая дважды выигрывала Google HashCode, в котором отжиг может использоваться в решениях._
...
Но теперь я знаю правильный способ. Мы можем просто подсмотреть код у [Psyho](https://cphof.org/profile/topcoder:Psyho) :) Я открыл его [решение](https://github.com/FakePsyho/cpcontests/blob/master/atcoder/ahc012/main.cpp) с последнего контеста Atcoder Heuristic, и знаете, что я увидел? Правильно, реализацию отжига.
...
На это трудно ответить, имея всего одно число. Сейчас мы никуда не торопимся, поэтому можем просто визуализировать данные, чтобы получить больше понимания насчёт того, что мы можем улучшить.
...
Обычно в контестах на оптимизацию лучше тратить время на пробу различных идей, добавление [новых видов переходов](http://tsp-basics.blogspot.com/), и так далее. Перед тем, как делать любые улучшения производительности, попробуйте просто увеличить TL и проверить, даёт ли это сильно лучший результат.
...
_Примечание по поводу перевода названия статьи: в оригинале обыгрывается многозначное слово converging, из-за чего название можно перевести и как "сходимость в имитации отжига", и как "сосредотачиваемся на имитации отжига"._
